...
public class HostsManager {
	...
	/**
	 * An array of size InstantType.values.length, it contains the number of
	 * host of type Instant type that have all slots full
	 */
	private int fullCount[];

	/**
	 * An array of size InstantType.values.length, it contains the number of
	 * host of type Instant type that have all slots empty
	 */
	private int emptyCount[];

	/**
	 * An array of size InstantType.values.length, it contains the number of
	 * host of type Instant type that have the least number of empty slot
	 */
	private int mostFilledCount[];

	/**
	 * An array of size InstantType.values.length, it contains the least number
	 * of empty slots of a host of type IntantType
	 */
	private int mostFilledEmptySlot[];

	/**
	 * Number of instant types available. In the context of our problem, it is
	 * assumed to be 3. This makes provision for new Instant types
	 */
	private static final int numIntances = InstantType.values().length;

	/**
	 * Default constructor, it initializes the different counters need by the
	 * Host manager
	 * 
	 */
	public HostsManager() {
		...
		for (int i = 0; i < numIntances; i++) {
			mostFilledEmptySlot[i] = Integer.MAX_VALUE;// Initialized to biggest
														// integer value
		}
	}

	/**
	 * Iterate through the different host and update the different counters
	 * based on host Instant type, free/full slots
	 * 
	 * This implementation assume full knowledge of the types of host instances
	 * (i.e. M1, M2, M3) A better solution might be to make it dynamic by
	 * storing the values of the InstantType and iterate through them
	 */
	void processHosts() {
		for (Host host : hosts) {
	 ...
	 * 
	 * @param hosts
	 *            use to set host that needs to be managed. Could be moved
	...
	public String toString() {
		return "EMPTY: M1 = " + emptyCount[0] + "; M2 = " + emptyCount[1] + "; M3 = " + emptyCount[2] + "\n"
				...
				+ "MOST FILLED: M1 = " + mostFilledCount[0] + ", " + mostFilledEmptySlot[0] + "; M2 = "
				+ mostFilledCount[1] + ", " + mostFilledEmptySlot[1] + "; M3 = " + mostFilledCount[2] + ","
				+ mostFilledEmptySlot[2];
	}
}
